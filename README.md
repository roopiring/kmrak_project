...
- 실서비스 환경에서 Apache Airflow, Kafka, Docker 기반의 데이터 수집·처리 파이프라인 구축 및 운영을 수행했으며, 레거시 PHP/MSSQL 시스템 현대화와 장애 대응 경험.
- 단순 기능 개발보다 운영 환경의 병목과 장애를 해결하는 데 강점을 가지고 있으며, 비동기 처리(asyncio), Bulk Insert/MERGE 기반 DB 적재 최적화 등을 통해 실제 서비스 성능 개선 및 운영 안정화를 수행
- 온프레미스 환경에서 XCP-ng 기반 가상화 인프라 구축, Airflow 분산 워커 운영, 중앙 로그 수집 환경 구성 등 데이터 플랫폼 운영 전반을 경험.
- Kafka 기반 실시간 유사물건추천 파이프라인 및 LLM 자동화 프로젝트를 진행하며 데이터/플랫폼 엔지니어링 역량을 확장.
