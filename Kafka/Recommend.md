...
- **성능 및 확장성 지표**
- 분산 처리 성능: 파티션 2개, 컨슈머 워커 2개를 구성하여 초당 20건 이상의 이벤트 처리 성능을 확보함.
...
2. 실시간 통신 및 병목 해결
- Problem: 프로듀서-브로커 간 통신 단절 및 대용량 이벤트 적재 시 지연 발생.
- Solution: 파티션 키(`item_id`+`user_id`) 전략 및 Consumer 병렬 처리 최적화.
- Result: 사용자 클릭 이벤트 발생 시 1초 내 추천 결과 반영 가능한 실시간성 확보.
