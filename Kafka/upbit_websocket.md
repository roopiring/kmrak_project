...
# 트레이드 오프에 대해서
## 1. flink와 sparkstreaming 중에서 실시간 데이터 처리인 flink 선택, sparkstreaming은 1초마다 마이크로배치형식이지만 flink는 실시간으로 데이터를 처리함 
## 2. 워터마크 5초의 이유 : 3초 5초 10초중에 5초로 선택함 이유는 3초는 너무 짧다고 느꼈고 10초는 너무 길다고 판단 그래서 5초정도의 딜레이는 유예해도 될것 같다는 자체 판단으로 채택
