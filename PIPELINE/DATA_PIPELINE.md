...
## Architecture
<img width="700" height="500" alt="image" src="https://github.com/roopiring/kmrak_project/blob/main/PIPELINE/Pipeline.PNG" />
...
- **Problem:** 데이터 수집이 중단되어 서비스 대시보드에 신규 데이터 미반영.
- **Solution:** 개편된 웹 아키텍처를 분석하여 DOM 구조 및 API 호출 규격을 즉시 리팩토링.
- **Result:** 서비스 중단 시간을 최소화하고 데이터 정합성을 유지함.

#### 2. 데이터 Row-by-Row 적재로 인한 DB 병목 및 속도 저하
- **Problem:** 빈번한 커넥션 오버헤드로 인한 DB 부하 발생 및 전체 수집 속도 저하.
- **Solution:** Staging 테이블에 Bulk Insert 후 운영 테이블에 MERGE 구문을 사용하여 변경분만 병합(Upsert).
- **Result:** 전체 수집 처리 속도를 획기적으로 향상시키고 DB 안정성 확보.

#### 3. 네트워크 불안정 및 외부기관 IP 차단 이슈
- **Problem:** 데이터 수집 로직 중단 및 파이프라인 가용성 저하.
- **Solution:** 기관 가이드라인에 맞춰 수집 속도를 조절하고, 차단 시 일정 시간 대기 후 재수집하는 로직 적용.
- **Result:** 현재 안정적 수집 중이며, 향후 상용 프록시 및 IP 로테이션 도입 계획.
